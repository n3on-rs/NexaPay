    ...
    <div className="relative">
      ...
      <input
        ...
        className={cn(
          ...
          Icon ? "pl-[3.25rem]" : "",
          rightElement ? "pr-14" : "",
          "focus:border-[#00d4aa] focus:ring-[3px] focus:ring-[#00d4aa]/10",
...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0b0b] flex font-inter text-white selection:bg-[#00d4aa] selection:text-black">
      <div className="absolute top-0 left-0 w-full h-[3px] bg-[#00d4aa] z-50 shadow-[0_0_10px_#00d4aa]" />
      <div className="fixed inset-0 pointer-events-none flex justify-center items-center">
        <div className="w-[800px] h-[800px] bg-[#00d4aa] opacity-5 blur-[120px] rounded-full" />
      </div>
          ...
          <div className="w-full max-w-md bg-[#111] border border-white/[0.06] rounded-[24px] p-8 lg:p-10 shadow-2xl relative overflow-hidden">
            ...
            {step === 1 && (
              <form onSubmit={onStep1} className="flex flex-col gap-5">
                ...
                <button type="submit" disabled={loading1} className="mt-2 w-full h-14 rounded-full bg-[#00d4aa] text-[#0b0b0b] font-extrabold text-lg flex items-center justify-center hover:bg-[#00d4aa]/90 transition-all disabled:opacity-50">
            ...
            {step === 2 && (
              <form onSubmit={onStep2} className="flex flex-col gap-5">
                ...
                {devOtp && <p className="text-center text-xs text-[#00d4aa] font-mono bg-[#00d4aa]/10 py-2 rounded-lg border border-[#00d4aa]/20">Dev OTP: {devOtp}</p>}
                ...
                <div className="flex justify-between gap-2">
                  {Array.from({ length: 6 }).map((_, i) => (
                    <input key={i} id={`fotp-${i}`} type="text" maxLength={1}
                      className="w-[52px] h-[64px] rounded-xl bg-[#111] border border-white/10 text-center text-2xl font-bold text-white focus:border-[#00d4aa] focus:ring-2 focus:ring-[#00d4aa]/20 outline-none transition-all"
                      value={otp[i] || ""}
                      onChange={(e) => {
                ...
                <div className="text-center text-sm font-medium">
                  {otpCooldown > 0 ? <span className="text-[#00d4aa]">Resend in {otpCooldown}s</span> : <span className="text-[#888]">Didn&apos;t receive it? Check your phone number</span>}
                </div>
                <button type="submit" disabled={loading2 || otp.length < 6} className="w-full h-14 rounded-full bg-[#00d4aa] text-[#0b0b0b] font-extrabold text-lg flex items-center justify-center hover:bg-[#00d4aa]/90 transition-all disabled:opacity-50">
            ...
            {step === 3 && (
              <form onSubmit={onStep3} className="flex flex-col gap-5">
                  ...
                  <div className="flex justify-between gap-2">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <input
                        ...
                        inputMode="numeric"
                        maxLength={1}
                        className="w-14 h-16 rounded-xl bg-[#111] border border-white/10 text-center text-2xl font-bold text-white focus:border-[#00d4aa] focus:ring-2 focus:ring-[#00d4aa]/20 outline-none transition-all"
                        value={newPin[i] || ""}
                        onChange={(e) => {
                          const val = e.target.value;
                          if (val) {
                ...
                <button type="submit" disabled={loading3} className="mt-2 w-full h-14 rounded-full bg-[#00d4aa] text-[#0b0b0b] font-extrabold text-lg flex items-center justify-center hover:bg-[#00d4aa]/90 transition-all disabled:opacity-50">
